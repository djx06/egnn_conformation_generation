...
def rmsd(src: torch.Tensor, tgt: torch.Tensor, mass: torch.Tensor=None) -> torch.Tensor:
    if mass is None:
...
def distance_loss(src: torch.Tensor, tgt: torch.Tensor):
    n = src.shape[0]
    loss = torch.tensor(0).type(torch.float32)
    for i in range(n):
        for j in range(n):
            loss += torch.pow((torch.pow(src[i]-src[j],2).sum() - torch.pow(tgt[i]-tgt[j],2).sum()),2)
    loss = loss/torch.pow(torch.tensor(n),2)
    return loss
